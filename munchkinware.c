# Encrypt all files under a given directory path using the RC4 encryption algorithm. All encrypted files will have the .rnsmd file extension. This snippet can be used to simulate ransomware.
# Usage: Call the EncryptFilesInGivenDir function, passing the unicode path to the directory containing the files to be encrypted.
